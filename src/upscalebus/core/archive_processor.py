                ...
                try:
                    ...
                    if not os.path.exists(target_path):
                        try:
                            ...
                            logger.info(f"[#process_log]{operation_type}新文件: {file} -> {target_file}")
                            success_count += 1
                        except OSError as e:
                        ...
                        if source_count == target_count:
                            try:
                                ...
                                action_func(temp_source, target_path)
                                logger.info(f"[#process_log]{operation_type}并覆盖: {file} -> {target_file}")
                                success_count += 1
                                logger.info(f"[#process_log]有效文件数量: {source_count}")
                            except Exception as e:
                                logger.info(f"[#update_log]文件操作或移至回收站失败: {str(e)}")
                                skip_count += 1
                                continue
                        else:
                            skip_count += 1
...
def check_archive(file_path):
    ...
    try:
        ...
        logger.error("[#process_log]错误: 未找到 7z 可执行文件。请确保 7z 已安装并添加到系统 PATH。")
            ...
            with open(history_file, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    try:
                        ...
                        logger.warning(f"[#process_log]跳过无效的历史记录行: {line}")
                        continue
        except Exception as e:
             logger.error(f"[#process_log]加载历史记录文件失败 {history_file}: {e}")
    return history
...
def process_corrupted_archives(directory, skip_checked=True, max_workers=4):
    ...
    def process_single_file(file_path):
        nonlocal processed
        try:
            logger.info(f"[#current_progress]正在检测: {os.path.basename(file_path)}")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                logger.info(f"[#update_log]任务执行失败 for {os.path.basename(file_path)}: {str(e)}")
                continue
            ...
            if not is_valid:
                ...
                try:
                    if os.path.exists(new_path):
                        os.remove(new_path)
                        logger.info(f"[#process_log]删除已存在的tdel文件: {os.path.basename(new_path)}")
                    os.rename(file_path, new_path)
                    logger.info(f"[#process_log]文件损坏,已重命名为: {os.path.basename(new_path)}")
                except Exception as e:
                    logger.info(f"[#update_log]重命名文件 {os.path.basename(file_path)} 时发生错误: {str(e)}")
            # else:
                # logger.info(f"[#process_log]文件 {os.path.basename(file_path)} 完好")
